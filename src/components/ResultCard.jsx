...
const ResultCard = ({item}) => {
  ...
  return (
    <div className='w-[22vw] h-80 relative bg-slate-800 rounded-xl overflow-hidden shadow-lg hover:scale-[1.02] hover:shadow-2xl transition-all duration-300'>
       ...
       <div id='bottom' className='flex justify-between gap-3 items-center w-full py-6 px-4 text-white absolute bottom-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent'>
        <h2 className='text-sm md:text-base capitalize font-semibold line-clamp-2 text-white'>{item.title}</h2>
        <button
        onClick={()=>{
         handleAddCollection(item)
        }}
        className='bg-cyan-500 hover:bg-cyan-600 active:scale-95 text-white rounded-lg px-4 py-2 font-medium cursor-pointer transition-all duration-200'>Save</button>
       </div>
    </div>
  )
}
